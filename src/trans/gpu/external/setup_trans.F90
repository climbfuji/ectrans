...
USE, INTRINSIC :: ISO_C_BINDING, ONLY: C_PTR, C_ASSOCIATED, C_SIZE_T, C_SIZEOF
...
IF( .NOT.D%LGRIDONLY ) THEN
  ...
  ! Cray compiler needs to copy those scalars explicitly. We don't do this
  ! by default because we will end up with wrong present counts in the end.
#ifdef _CRAYFTN
  !$ACC ENTER DATA COPYIN(R%NSMAX,R%NTMAX,R%NDGL,R%NDGNH) ASYNC(1)
  !$ACC ENTER DATA COPYIN(D%NUMP,D%NDGL_FS) ASYNC(1)
#endif
  !$ACC WAIT(1)
#endif
#ifdef OMPGPU
