...
! Handle KSORT argument, which allows to change the order of fields in PGP (but only when NPROC = 1)
...
! Copy for single MPI task
IF(NPROC == 1) THEN
  !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JKGLO,IEND,IOFF,IBL,JFLD,JROF)
  ! Loop over all grid point NPROMA blocks
  DO JKGLO=1,D%NGPTOT,KPROMA
    IEND = MIN(KPROMA,D%NGPTOT-JKGLO+1) ! Get length of this NPROMA block
    IOFF = JKGLO-1
    IBL  = (JKGLO-1)/KPROMA+1 ! Get index of this NPROMA block
    ! Unpack PGPG into PGP
    DO JFLD=1,KFDISTG
      DO JROF=1,IEND
...
ELSEIF(KFDISTG>0) THEN
  ! Test if values in KFROM are all the same, i.e. just one task is sending all fields
  LLSAME=.TRUE.
  IFROM=KFROM(1)
  DO JFLD=2,KFDISTG
  ...
  ! Determine how many fields this task will be sending
  IMYFIELDS = 0
  DO JFLD=1,KFDISTG
  ...
  ! If this task has any fields to send, prepare send buffer
  CALL GSTATS(1663,0)
  IF(IMYFIELDS > 0) THEN
